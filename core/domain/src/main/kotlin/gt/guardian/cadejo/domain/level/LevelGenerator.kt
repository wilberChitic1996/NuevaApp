...
object LevelGenerator {
    ...
    fun generate(seed: Long, levelIndex: Int, startingScore: Int = 0): GameState {
    ...
    private fun enemyArchetype(i: Int, levelIndex: Int): Pair<SpiritKind, PatternType> = when {
        ...
        else -> if (levelIndex % 2 == 0) SpiritKind.LLORONA to PatternType.CHASE
        else SpiritKind.SOMBRERON to PatternType.PATROL
    }

    /** A small local loop around [start] for patrolling spirits. */
    private fun patrolRoute(start: Hex, walls: Set<Hex>, onBoard: List<Hex>): List<Hex> {
    ...
    private fun isReachable(radius: Int, walls: Set<Hex>, from: Hex, goal: Hex): Boolean {
